		...
		FROM (
			...
			FROM (
                ...
                FROM (
                    ...
                    AND DATE(oo.shipped) >= {{ yearSelected.value + '-' + monthSelected.value.toString().padStart(2, 0) + '-01'}}
                    AND DATE(oo.shipped) <= {{ yearSelected.value + '-' + monthSelected.value.toString().padStart(2, 0) + '-' + new Date(yearSelected.value,monthSelected.value,0).getDate().toString()}}
